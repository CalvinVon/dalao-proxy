...
class WsConnector {
    ...
    connect() {
        ...
        if (env !== "production") {
            socketUrl = "ws://localhost:20001/ws";
        }
        ...
        ws.onclose = ev => {
            ...
            notification.open({
                ...
                description:
                    "Connection lost, refresh the page to connect server.",
                onClick: () => {
    ...
    listen(type, listener, scope, once) {
        if (scope && scope instanceof React.Component) {
            const destroy = scope.componentWillUnmount;
            scope.componentWillUnmount = function () {
