<template>
	<div class="monitor">
		<div class="monitor-header">
			...
			<div class="request-filter flex flex-content-between">
				...
				<button class="btn-clear"
				        @click="clearAllData">Clear All</button>
			</div>
		...
		<div class="monitor-footer">
			...
			<div class="divider"></div>
			Finished {{ allDataTime | unitFormat('time') }}

			<span class="float-right">
				<span>@calvin_von/proxy-plugin-monitor </span>
				<a href="https://github.com/CalvinVon/dalao-proxy/tree/feat-plugiu-request-monitor/packages/%40calvin_von/proxy-plugin-monitor">Github</a>
			</span>
		</div>
	</div>
</template>
	...
	computed: {
		...
		filteredData() {
			return this.monitorData.filter(item => {
				...
				if (this.textFilter) {
					CONDITIONS.push(new RegExp(this.textFilter).test(item.url));
					console.log(new RegExp(this.textFilter));
					console.log(item.url);
					console.log(new RegExp(this.textFilter).test(item.url));
				}
